...
MouseHandler::MouseHandler(
    std::function<void(int, float, float, float, float)> handler)
    : handleEvent(handler) {}

MouseHandler::MouseHandler(std::function<void(int, float, float)> handler)
    : handleEvent(
          [handler](int mouseFlags, float clampedx, float clampedy, float,
                    float) { handler(mouseFlags, clampedx, clampedy); }) {}

MouseHandler::MouseHandler(std::function<void(int)> handler)
    : handleEvent([handler](int mouseFlags, float, float, float, float) {
        handler(mouseFlags);
      }) {}

MouseHandlerContainer::MouseHandlerContainer(float xi, float yi, float xClampi,
                                             float yClampi)
    : x(xi),
      y(yi),
      xClamp(xClampi),
...
void MouseHandlerContainer::setPosition(float xi, float yi) {
  ...
  clampedy = glm::clamp(clampedy - diffy, -yClamp,
                        yClamp);  // invert y diff since the mouse uses a
...
void MouseHandlerContainer::handle() {
  ...
  flags &= ~(MOUSE_LEFTBUTTON_PRESSED | MOUSE_RIGHTBUTTON_PRESSED |
             MOUSE_SCROLLED_UP_PRESSED | MOUSE_SCROLLED_DOWN_PRESSED);
  diffx = diffy = 0;
}