...
ProgressBar::ProgressBar(Scripts &scripti)
    : playedProgressBar(ModelLoader::getUnitSquareShape()),
      ...
      playPauseButton("res/shapes/pause.shp", "res/shapes/play.shp", -1, -1,
                      0.1),
      editButton("res/shapes/hammer.shp", "res/shapes/confirm.shp", -1, 0.9,
                 0.1, glm::radians(45.0F)),
      script(scripti) {
...
void ProgressBar::attachControls(KeyHandlerContainer &keyContainer,
  ...
  keyContainer.emplace_handler(
      GLFW_KEY_Y, [this]() { script.rotateActiveKeyframeBy(0.005, 0, 0); });
  keyContainer.emplace_handler(
      GLFW_KEY_H, [this]() { script.rotateActiveKeyframeBy(-0.005, 0, 0); });
  keyContainer.emplace_handler(
      GLFW_KEY_U, [this]() { script.rotateActiveKeyframeBy(0, 0.005, 0); });
  keyContainer.emplace_handler(
      GLFW_KEY_J, [this]() { script.rotateActiveKeyframeBy(0, -0.005, 0); });
  keyContainer.emplace_handler(
      GLFW_KEY_I, [this]() { script.rotateActiveKeyframeBy(0, 0, 0.005); });
  keyContainer.emplace_handler(
      GLFW_KEY_K, [this]() { script.rotateActiveKeyframeBy(0, 0, -0.005); });
  ...
  mouseContainer.emplace_handler([this](int mouseFlags, float clampedx,
                                        float clampedy, float diffx,
                                        float diffy) {
    clampedx /= SCREEN_WIDTH;
    clampedy /= SCREEN_HEIGHT;
    if (editButton.state) {
    ...
    auto inKeyframeHeightRange = [](auto clampedy) {
      return clampedy < PROGRESS_BAR_LOWER_BOUND +
                            KEYFRAME_INDICATOR_HEIGHT_DIFF +
                            PROGRESS_BAR_HEIGHT &&
             clampedy >
                 PROGRESS_BAR_LOWER_BOUND - KEYFRAME_INDICATOR_HEIGHT_DIFF;
    };
    auto inKeyframeWidthRange = [this, clampedx](auto keyframeTimestamp) {
      return clampedx < PROGRESS_BAR_LEFT_BOUND +
                            KEYFRAME_INDICATOR_WIDTH_HALFED +
                            keyframeTimestamp * PROGRESS_BAR_LENGTH /
                                script.getMaximumTime() &&
             clampedx > PROGRESS_BAR_LEFT_BOUND -
                            KEYFRAME_INDICATOR_WIDTH_HALFED +
                            keyframeTimestamp * PROGRESS_BAR_LENGTH /
                                script.getMaximumTime();
    ...
    if ((mouseFlags & MOUSE_LEFTBUTTON_PRESSED) &&
        inKeyframeHeightRange(clampedy)) {
      auto target = std::find_if(frames.cbegin() + 1, frames.cend() - 1,
                                 inKeyframeWidthRange);
      ...
      if (draggingKeyframeDefinitionCountdown != 10 &&
          ...
          clampedx > PROGRESS_BAR_LEFT_BOUND) {
        draggingKeyframeDefinitionCountdown = 10;
        script.addKeyframeAt((clampedx - PROGRESS_BAR_LEFT_BOUND) /
                             PROGRESS_BAR_LENGTH * script.getMaximumTime());
      }
    }
    ...
    if (mouseFlags & MOUSE_LEFTBUTTON_HOLD) {
      if (draggingKeyframeDefinitionCountdown) {
        if (draggingKeyframeDefinitionCountdown > 0) {
          ...
          script.moveActiveKeyframeBy(
              glm::vec3(-diffx * rate, -diffy * rate, zmovement));
        }
      } else {
        script.setActiveTimestamp(
            std::max(0.0F, (clampedx - PROGRESS_BAR_LEFT_BOUND) /
                               PROGRESS_BAR_LENGTH) *
            script.getMaximumTime());
      }
    }
...
void ProgressBar::draw(ShaderProgram &shader) {
  if (editButton.state) {
    ...
    unplayedProgressBar.setTransformMatrix(
        glm::mat3{{PROGRESS_BAR_LENGTH, 0, 0},
                  {0, PROGRESS_BAR_HEIGHT, 0},
                  {PROGRESS_BAR_LEFT_BOUND, PROGRESS_BAR_LOWER_BOUND, 1}});
    unplayedProgressBar.draw(shader);
    // Draws keyframes.
    ...
    for (auto iter = frames.cbegin() + 1; iter < frames.cend() - 1; ++iter) {
      keyframeObject.setTransformMatrix(glm::mat3{
          ...
          {PROGRESS_BAR_LEFT_BOUND - KEYFRAME_INDICATOR_WIDTH_HALFED +
               *iter * PROGRESS_BAR_LENGTH / script.getMaximumTime(),
           PROGRESS_BAR_LOWER_BOUND - KEYFRAME_INDICATOR_HEIGHT_DIFF, 1}});
