...
Keyframe::Keyframe(float timestampi, float xi, float yi, float zi,
                   const Quaternion &orientationi)
    : timestamp(timestampi), x(xi), y(yi), z(zi), orientation(orientationi) {}

void Keyframe::setTimestamp(float timestampi) { timestamp = timestampi; }

void Keyframe::moveX(float dx) { x += dx; }

void Keyframe::moveY(float dy) { y += dy; }

void Keyframe::moveZ(float dz) { z += dz; }
...
/**
 * Returns the transcation matrix of the keyframe. Including rotation, scale(Not
 * implemented), and transcations.
 */
