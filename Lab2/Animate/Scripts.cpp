...
Scripts::Scripts(Scripts &&another)
    : activeKeyframeIndex(another.activeKeyframeIndex), useCatmullRomInterpolate(another.useCatmullRomInterpolate), timestamps(std::move(another.timestamps)), keyframes(std::move(another.keyframes))
{
}
...
void Scripts::addKeyframeAt(float time)
{
    ...
    addKeyframe(Keyframe(time, transform[3][0], transform[3][1], transform[3][2], Quaternion()));
    rearrangeKeyframes();
...
Scripts Scripts::loadScript(const char *filename)
{
    ...
    while (parser.isValid())
    {
        ...
        if (loaded == 8)
        {
            loadedScript.addKeyframe(Keyframe(data[0], data[1], data[2], data[3], Quaternion(data[4], data[5], data[6], data[7])));
        }
        else
        {
            std::cerr << "ScriptsLoader: Warning: Error happened parsing the script: " << filename << std::endl;
