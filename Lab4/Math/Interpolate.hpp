...
T simpleLinearInterpolate(float t, const T& val1, const T& val2) {
   ...
   *         \ 1   0 /   \ val2 /
   */
  const float linearM[2][2] = {{-1, 1}, {1, 0}};
  return linearInterpolate(t, val1, val2, linearM);
}

/**
 * Further cubic interpolate can simply pass the coresponding M into this
 * function.
 */
template <typename T>
T cubicInterpolate(float t, const T& val1, const T& val2, const T& val3,
                   const T& val4, const float M[4][4]) {
  /*  T                       M                 G
...
template <typename T>
T catmullRomInterpolate(float t, const T& val1, const T& val2, const T& val3,
                        const T& val4) {
...
auto catmullRomInterpolate(float t, Iterator begin) {
  return catmullRomInterpolate(t, *begin, *(begin + 1), *(begin + 2),
                               *(begin + 3));
}

template <typename T>
T uniformNonrationalBasisSplineInterpolate(float t, const T& val1,
                                           const T& val2, const T& val3,
                                           const T& val4) {
  const float catmullRomM[4][4] = {
...
auto uniformNonrationalBasisSplineInterpolate(float t, Iterator begin) {
  return uniformNonrationalBasisSplineInterpolate(t, *begin, *(begin + 1),
                                                  *(begin + 2), *(begin + 3));
}
